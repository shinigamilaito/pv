class PrintingIncomeReportsController < ApplicationController
  def index
    sales = PrintingSale
                .all

    @total = PrintingSale.parcial(sales)
    @sales = sales
                 .paginate(page: params[:page], per_page: self.elements_per_page)
                 .order(updated_at: :desc)

    @index = obtain_index(params[:page].to_i)
    @module = "incomes_printing"
    ...
    if start_date_string == "" || end_date_string == ""
      ...
      if start_date <= end_date
        ...
        respond_to do |format|
          ...
          format.xlsx {
            @sales_xlsx = sales
            @title = "Ingresos por Ventas en Imprenta: #{start_date} - #{end_date}"
            response.headers['Content-Disposition'] = 'attachment; filename="#{@title}.xlsx"'
            render xlsx: @title, template: 'printing_income_reports/index'
          }
        end
      else
        head :no_content
  ...
  def employee_incomes
    ...
    respond_to do |format|
      ...
      format.xlsx {
        ...
        @title = "Ingresos por Ventas en Imprenta - Empleado: #{name_employee}"
        @sales_xlsx = sales
        response.headers['Content-Disposition'] = 'attachment; filename="#{@title}.xlsx"'
        render xlsx: @title, template: 'printing_income_reports/index'
      }
      ...
      if lower_limit < upper_limit
        ...
        respond_to do |format|
          ...
          format.xlsx {
            ...
            @title = "Ingresos por Ventas en Imprenta: #{lower_quantity} - #{upper_quantity}"
            @sales_xlsx = sales
            response.headers['Content-Disposition'] = 'attachment; filename="#{@title}.xlsx"'
            render xlsx: @title, template: 'printing_income_reports/index'
          }
  ...
  def ticket_incomes
    ...
    if ticket == ""
      ...
      respond_to do |format|
        ...
        format.xlsx {
          @title = "Ingresos por Ventas en Imprenta - Ticket: #{ticket}"
          @sales_xlsx = sales
          response.headers['Content-Disposition'] = 'attachment; filename="#{@title}.xlsx"'
          render xlsx: @title, template: 'printing_income_reports/index'
        }
